...
struct Treap {
	struct Tree {int l,r,size,exist,sum;} tree[maxn<<1];
	int size;
...
#define size(x) tree[x].size
	int newnode() {tree[++size]=(Tree) {0,0,1,1,1};return size;}
	void push_up(int x) {
...
#define mid ((left+right)>>1)
	void modify(int &index,int left,int right,int Left,int Right,int v) {
		if(right<Left||left>Right)return;
		if(!index)index=++size;
		if(Left<=left&&Right>=right) {
			int v2=1ll*v*v%mod;
			add(S.s1,v),add(S.s3,1ll*v2*v%mod);
			if(v>0)add(S.s0,1),add(S.s2,v2);
			else add(S.s0,mod-1),add(S.s2,mod-v2);
			return;
		}
	...
	Status query(int index,int left,int right,int tar) {
		if(!index||left>tar||right<tar)return (Status) {0,0,0,0};
...
int main() {
	...
	for(int j=1; j<=m; j++) {
		...
		if(opt==1) {
			x=rk[x],y=rk[y];
			Status s= {0,0,0,0};
			for(int i=x; i>0; i-=lowbit(i))s=s+st.query(rt[i],1,step,y);
