...
#define size(x) tree[x].size 
	...
	void remove(int val) {
		...
		int pos=rebuild(now);
		if(now==root)root=pos;
		else tree[fa].child[side]=pos;
	}
	int rank(int val) {
		int now=root,ans=0;
		while(now) {
	...
	int pre(int val) {
		...
		while(now) {
			if(val(now)<val)ans=max(ans,val(now)),now=rs(now);
			else now=ls(now);
		}
		return ans;
	}
	int suc(int val) {
		...
		while(now) {
			if(val(now)>val)ans=min(ans,val(now)),now=ls(now);
			else now=rs(now);
		}
		return ans;
	}
...
int main() {
	...
	for(int i=1; i<=n; i++) {
		int opt=Get_Int();
		if(opt==1)treap.insert(treap.root,Get_Int());
		if(opt==2)treap.remove(Get_Int());
		if(opt==3)printf("%d\n",treap.rank(Get_Int()));
