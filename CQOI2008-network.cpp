...
int main() {
	...
	for(int i=1; i<=n; i++) {
		...
		if(x=='Y')dinic.AddEdge(i,2*(n+1),1);
	}
	for(int i=1; i<=n; i++)
		for(int j=1; j<=n; j++) {
			char x=' ';
			while(!isalpha(x))x=getchar();
			if(x=='Y')dinic.AddEdge(i,n+1+j,1);
		}
	for(int i=1; i<=n; i++) {
		dinic.AddEdge(S,i,1);
		link[i]=dinic.AddEdge(n+1+i,T,0);
	}
	dinic.AddEdge(2*(n+1),T,INT_MAX/2);
	for(int k=1; k<=n; k++) {
		for(int i=1; i<=n; i++)dinic.edges[link[i]].cap++;
