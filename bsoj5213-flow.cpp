...
int main() {
	...
	tot=0;
	for(int i=1; i<=m; i++) {
		int x=Get_Int(),y=Get_Int(),l=Get_Int(),h=Get_Int();
		dinic.AddEdge(x,y,h-l);
		sum[y]+=l;
		sum[x]-=l;
	}
	int S=n+1,T=n+2;
	for(int i=1; i<=n; i++)
		if(sum[i]>0) {
			dinic.AddEdge(S,i,sum[i]);
			tot+=sum[i];
		} else if(sum[i]<0)dinic.AddEdge(i,T,-sum[i]);
	printf("%s\n",dinic.maxflow(S,T)==tot?"Yes":"No");
	return 0;
} 
